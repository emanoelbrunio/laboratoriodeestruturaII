...
int duplo_hash(int posicao, int chave, int i, int TABLE_SIZE) {
    int h2 = multiplicacao(chave, TABLE_SIZE);

    if (h2 == 0){
        h2 = h2 + 1;
    }
    return ((posicao + i * h2) & 0x7FFFFFFF) % TABLE_SIZE;
}
    ...
    if (ha->itens[posicao] != NULL){
        if (ha->itens[posicao] == -1){
            ...
            *novo_aluno = aluno1;
            ...
            do {
                aux = duplo_hash(posicao, chave, i, ha->TABLE_SIZE);
                i++;
            } while(ha->itens[aux] != NULL || ha->itens[aux] != -1);
...
int buscar(Hash* ha, int mat, Aluno* aluno1){
    ...
    do {
        aux = duplo_hash(posicao, mat, i, ha->TABLE_SIZE);
        if (ha->itens[aux] == NULL || ha->itens[aux] == -1){
            return 0;
        }
        if (ha->itens[aux]->matricula == mat){
